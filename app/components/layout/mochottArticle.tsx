  ...
  const MctAtc = useCallback(
    ({ children: argsChild, base, isTableDirectry }: _mochottArticle_Props) => {
      ...
      const children = useMemo(
        () => (Array.isArray(argsChild) ? argsChild : [argsChild]),
        [argsChild],
      );
      const tableDirectryResult = useMemo(() => {
        if (isTableDirectry) {
          if (
      ...
      if (tableDirectryResult) return tableDirectryResult;
      return useMemo(
        () =>
          children.map((item, i) => {
            const key = `${base}-${i}`;
                      ...
                      switch (mark.type) {
                        case "link":
                          ...
                          if (mark.attrs) {
                            ...
                            if (mark.attrs.class)
                              attr.className = mark.attrs.class;
