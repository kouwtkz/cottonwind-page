...
export async function SetAccessToken(
  { env, code, code_verifier, client_id, redirect_uri, basicAuthorization }
    : { env: any, code: string, client_id: string, redirect_uri: string, code_verifier: string, basicAuthorization: string }) {
...
export async function RefreshAccessToken(
  { env, refresh_token, user, client_id, basicAuthorization }
    : { env: any, refresh_token: string, user?: TwitterUserType, client_id: string, basicAuthorization: string }) {
  const oauth2_access_options =
  {
    refresh_token,
...
export async function SyncToken(env: any) {
...
export async function RevokeToken({ env, token, token_type_hint = "access_token", basicAuthorization }: { env: any, token: string, token_type_hint?: "access_token" | "refresh_token", basicAuthorization: string }) {
